...
const PlaceItem = ({
  ...
  const deletePlaceHandler = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/places/${P_ID}`,
        {
          method: "DELETE",
          headers: {
  ...
  const deleteModalMain = (
    <React.Fragment>
      <p>Do You Want to proceed For Deleting The Specified Place </p>
      <p style={{ border: "3px solid #c20e0e4c " }}>
        <b>Caution : </b>Once a Place is Deleted It cannot be Undone
      </p>
    </React.Fragment>
  );
  return (
